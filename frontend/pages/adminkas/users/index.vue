            ...
            <div class="reviews__row">
              <div class="table-responsive">
                ...
                >
                  ...
                  >
                    <template slot-scope="scope">
                      <template v-if="scope.row.girls.length > 0">
                        <GirlLink
...
export default {
  ...
  asyncData({store, params}) {
    return store.dispatch('users/listUsers', {
      sort: 'id',
      sortOrder: 'desc'
    })
      .then((response) => {
  ...
  methods: {
    onSelect(user, column) {
      if(['link'].indexOf(column.property) !== -1 ) return;

      this.$router.push('/adminkas/users/' + user.id + '/mans')
    },
    confirmEmail(user) {
      this.$store.dispatch('users/confirmEmail', user.id)
      .then(() => {
    ...
    async loadMore(page) {
      ...
      if(page == 1) {
        await this.$store.dispatch('users/listUsers', params)
      } else {
        await this.$store.dispatch('users/loadMoreUsers', params)
      }
    ...
    deleteUser(user_id) {
      MessageBox.confirm('Пользователя нельзя будет восстановить!', 'Внимание', {
        confirmButtonText: 'Удалить',
        cancelButtonText: 'Отменить',
        type: 'warning'
      }).then(() => {
        this.isBusy = true;
        this.$store.dispatch('users/removeUser', user_id)
          .then((response) => {
            this.isBusy = false;
            Message.success({
              showClose: true,
