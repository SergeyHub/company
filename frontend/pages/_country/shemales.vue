  ...
  export default {
    ...
    data() {
      return {
        ...
        page: 1,
        breadcrumbs: [{
          text: this.$t('titles.shemales')
        }]
      }
    },
    // head() {
    //   let title = this.$t('titles.shemales');
    //   return {
    //     title: title,
    //   }
    // },
    async asyncData({ store, params, error }) {
      ...
      await store.dispatch('girls/setCurrentSection', 'shemales');
      ...
      let response = await store.dispatch('girls/fetchShemales', filter_params)
      return {
        loading: false,
    ...
    methods: {
      loadMore() {
        ...
        this.$store.dispatch('girls/fetchShemales', params)
          .then((response) => {
            this.busy = false;
          })
      }
