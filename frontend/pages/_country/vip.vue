  ...
  export default {
    ...
    data() {
      return {
        ...
        page: 1,
        breadcrumbs: [{
          text: this.$t('titles.vip')
        }]
      }
    },
    // head() {
    //   let title = this.$t('titles.vip');
    //   return {
    //     title: title,
    //   }
    // },
    async asyncData({ store, params, error }) {
      ...
      await store.dispatch('girls/setCurrentSection', 'vip');
      ...
      let response = await store.dispatch('girls/fetchVipGirls', filter_params)
      return {
        loading: false,
