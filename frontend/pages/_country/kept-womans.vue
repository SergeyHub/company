  ...
  export default {
    ...
    data() {
      return {
        ...
        page: 1,
        breadcrumbs: [{
          text: this.$t('titles.kept_womans')
        }]
      }
    },
    // head() {
    //   let title = this.$t('titles.kept_womans');
    //   return {
    //     title: title,
    //   }
    // },
    async asyncData({ store, params, error }) {
      ...
      await store.dispatch('girls/setCurrentSection', 'kept-womans');
      ...
      let response = await store.dispatch('girls/fetchKeptWomans', filter_params)
      return {
        loading: false,
    ...
    methods: {
      loadMore() {
        ...
        this.$store.dispatch('girls/fetchKeptWomans', params)
          .then((response) => {
            this.busy = false;
          })
      }
