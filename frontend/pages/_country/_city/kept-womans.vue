...
<script>
  ...
  export default {
    ...
    //
    //   let description = this.$i18n.t('descriptions.home_page_kept_womans_city')
    ...
    async asyncData({req, res, store, params, error}) {
      ...
      store.dispatch('girls/setCurrentSection', 'kept-womans');
      ...
      let response = await store.dispatch('girls/fetchKeptWomans', query);
      return {
        loading: false,
