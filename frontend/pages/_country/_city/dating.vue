...
<script>
  ...
  export default {
    ...
    //
    //   let description = this.$i18n.t('descriptions.home_page_dating_city')
    ...
    async asyncData({req, res, store, params, error}) {
      ...
      store.dispatch('girls/setCurrentSection', 'dating');
      ...
      let response = await store.dispatch('girls/fetchDatingGirls', query);
      return {
        loading: false,
