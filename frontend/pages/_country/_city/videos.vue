...
export default {
  ...
  head() {
    let title = this.$t('titles.title_videos_city');
    title = title.replace(/%city%/gi, (this.city ? this.city.name : ''));

    let description = this.$t('descriptions.videos_city').replace('%city%', this.city.name)
  ...
  async asyncData({store, params, error}) {
    ...
    if (store.state.city) {
      filter_params.city = store.state.city.id
    }
