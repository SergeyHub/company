...
<script>
  ...
  export default {
    ...
    //
    //   let description = this.$i18n.t('descriptions.home_page_shemales_city')
    ...
    async asyncData({req, res, store, params, error}) {
      ...
      store.dispatch('girls/setCurrentSection', 'shemales');
      ...
      let response = await store.dispatch('girls/fetchShemales', query);
      return {
        loading: false,
